    ...
    _helpers.orderFields = function (frmbFields) {
        ...
        for (var i = fieldOrder.length - 1; i >= 0; i--) {
            var field = frmbFields.filter(function (field) {
                if (field.attrs.name !== undefined && (field.attrs.name).startsWith('custom')) {
                    return field.attrs.name === fieldOrder[i];
                } else {
...
(function ($) {
    var FormBuilder = function FormBuilder(options, element) {
        ...
        $.each(opts.customFields, function (key, item) {
            frmbFields.push(item);
        });
